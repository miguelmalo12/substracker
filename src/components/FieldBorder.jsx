...
function FieldBorder({ title, type, options, defaultValue, placeholder }) {
  ...
  return (
    <div className="flex items-center justify-between pt-4 pb-2 border-b-2">
      ...
      {type === "select" ? (
        <Select.Root className="w-full p-2 text-right border-none" defaultValue={defaultValue}>
          <Select.Trigger className="inline-flex items-center justify-center h-8 gap-1 leading-none rounded cursor-pointer drop-shadow focus:drop-shadow focus:px-4" >
            ...
            <Select.Content className="overflow-hidden bg-white rounded drop-shadow">
              ...
              <Select.Viewport className="p-4">
                {options.map((option, index) => (
                  <Select.Item key={index} value={option} className='relative flex items-center h-6 px-6 text-xs leading-none rounded select-none hover:text-white hover:bg-primary hover:border-success'>
